...
type SimpleSection = "basics" | "person" | "interview" | "document" | "docx";

function sectionForDiffPath(path: string): SimpleSection | null {
  if (
    path === "name" ||
    path.startsWith("modelIdentity") ||
    path.startsWith("caseMetadataDeps")
  ) {
    return "basics";
  }

  if (
    path === "witnessMetadataFields" ||
    path.startsWith("witnessMetadataFields.")
  ) {
    return "person";
  }
...
export function StatementTemplateSimpleView() {
  ...
  const activeDiffPath = activeDiff?.path ?? null;
  ...
  useEffect(() => {
    ...
    const nextSection = sectionForDiffPath(activeDiffPath);
    if (nextSection && nextSection !== sectionRef.current) {
      pendingScrollPath.current = activeDiffPath;
      route.setSection(nextSection, "replace");
      return;
    }

    scrollToDiffPath(activeDiffPath);
  }, [activeDiffPath, scrollToDiffPath]);
  ...
  useEffect(() => {
    ...
    const target = sectionForDiffPath(path);
    if (target && target !== section) {
      return;
    }

    pendingScrollPath.current = null;
    scrollToDiffPath(path);
  }, [scrollToDiffPath, section]);
  ...
  return (
    <div className="space-y-4">
      <Tabs
        value={section}
        onValueChange={(value) => route.setSection(value as SimpleSection)}
      >
        <TabsList>
          ...
          {renderNativeDiffField(
            ...
            <div className="space-y-3 rounded-md border p-3">
              ...
              <Input
                ...
                onBlur={() =>
                  setTouchedTemplateNameKey(currentTemplateTouchKey)
                }
                disabled={isBusy}
            ...
            <div className="space-y-3 rounded-md border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Who the model is for this interview. This leads the interview,
                  the phase review, and the written statement.
                </p>
              </div>
              <Textarea
                rows={3}
                ...
                <div className="space-y-3 rounded-md border p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Case field ids the interview and formalizer may see. An
                        empty list means no case facts are injected.
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="outline">
                        {draftConfig.caseMetadataDeps.length}
                      </Badge>
                      <Button
                        type="button"
                        variant="outline"
                    ...
                    {renderNativeDiffField(
                      ...
                      <Input
                        ...
                        onChange={(event) => {
                          ...
                          const shouldAutoGenerateId =
                            isWitnessFieldIdAutoManaged(field.label, field.id);
                          next[index] = {
                            ...next[index],
                            label: nextLabel,
        ...
        <TabsContent value="document" className="space-y-4 pt-4">
          ...
          {!hasPendingSections ? (
            <DynamicFieldsEditor
              ...
              getItemClassName={(_, index) =>
                getItemClassName(`sections.${index}`)
              }
              headerActions={
                pendingAiPatchDiffs.some((diff) =>
