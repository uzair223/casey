...
import { getStatementNotes } from "@/lib/supabase/queries";
import {
  createStatementNote,
  deleteStatementNote,
...
export function StatementNotesCard({
  statementId,
  tenantId,
  canPinNotes,
}: StatementNotesCardProps) {
  const { user } = useUser();
  ...
  >(
    async () => {
      ...
      return getStatementNotes(statementId);
    },
    [statementId],
  ...
  const onCreateNote = async () => {
    ...
    const noteId = await createStatementNote({
      tenantId,
      statementId,
    ...
    await apiFetch("/api/notifications/mentions", {
      method: "POST",
      body: JSON.stringify({ kind: "statement", noteId }),
    }).catch((error) => {
  ...
  const onDeleteNote = async (note: CollaborationNoteView) => {
    ...
    await deleteStatementNote(note.id);
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Textarea
            ...
            placeholder="Add an internal note for this statement"
            rows={3}
          />
