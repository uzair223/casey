...
export type MentionNotificationSourceKind = "case";
...
export async function SERVERONLY_getMentionNotificationDispatchContext(
  _kind: MentionNotificationSourceKind,
  noteId: string,
  ...
  const { data: note, error: noteError } = await supabase
    .from("case_notes")
    .select("id, tenant_id, case_id, statement_id, body, author_user_id")
    .eq("id", noteId)
    .maybeSingle();
  ...
  return {
    ...
    noteType: note.statement_id ? "statement_note" : "case_note",
    noteId: note.id,
    caseId: note.case_id,
    caseTitle: caseRow.title,
    statementId: note.statement_id,
    noteBody: note.body,
